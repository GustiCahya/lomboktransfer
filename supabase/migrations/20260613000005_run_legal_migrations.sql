-- ============================================================
-- Fix Missing Legal Tables: company_documents, contracts, 
-- data_access_log, and data_deletion_requests
-- 
...
-- Policies (Wrapped in DO blocks to avoid errors if they already exist)
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_policies WHERE policyname = 'Allow all read access on company_documents') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_policies WHERE policyname = 'Allow all read access on contracts') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_policies WHERE policyname = 'Allow all read access on data_access_log') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_policies WHERE policyname = 'Allow all read access on data_deletion_requests') THEN
...
AND table_name IN ('company_documents', 'contracts', 'data_access_log', 'data_deletion_requests');
